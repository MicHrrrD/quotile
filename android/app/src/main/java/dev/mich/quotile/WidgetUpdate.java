...
public final class WidgetUpdate {
    ...
    /** Send native host animations once, keeping all exact Fold size variants. */
    static void beginReveal(Context c, WidgetState state) {
        AppWidgetManager m = AppWidgetManager.getInstance(c);
        for (Class<?> provider : new Class<?>[]{SlimWidgetProvider.class, DetailWidgetProvider.class})
            for (int id : m.getAppWidgetIds(new ComponentName(c, provider))) {
                RemoteViews views = prepare(c, m, id, state, true);
                if (views != null) m.updateAppWidget(id, views);
            }
    }
    public static void update(Context c, AppWidgetManager m, int id) {
        WidgetMotion.cancel();
        RemoteViews views = prepare(c, m, id, new QuotaStore(c).state(), false);
        if (views != null) m.updateAppWidget(id, views);
    }
    private static RemoteViews prepare(Context c, AppWidgetManager m, int id, WidgetState state, boolean reveal) {
        ...
        try {
            if (sizes != null && !sizes.isEmpty()) {
                ...
                for (SizeF size : sizes) {
                    ...
                    views.put(size, view(c, id, info.provider, Math.round(size.getWidth()), Math.round(size.getHeight()), state, dark, reveal));
                }
                if (!views.isEmpty()) return new RemoteViews(views);
            }
            ...
            RemoteViews portrait = view(c, id, info.provider, minW, maxH, state, dark, reveal);
            RemoteViews landscape = view(c, id, info.provider, maxW, minH, state, dark, reveal);
            ...
            RemoteViews fallback = view(c, id, info.provider, 300, detail ? 140 : 64, state, dark, reveal);
            return fallback;
        }
    }
    private static RemoteViews view(Context c, int id, ComponentName provider, int width, int height, WidgetState state, boolean dark, boolean reveal) {
        ...
        RemoteViews rv = WidgetRenderer.remoteViews(c, width, height, state, dark, QuotaSync.isRunning(),
                android.animation.ValueAnimator.areAnimatorsEnabled(), reveal);

...
public final class ManualModeTests extends Instrumentation {
    ...
    @Override public void onStart() {
        ...
        try {
            ...
            result.putString("stream", "PASS: settings launch, reopen, launcher callbacks and theme changes remain offline with auto OFF; opt-in defaults and task cancellation; widget broadcast does not launch an Activity; 24 base native widget render cases plus mixed-script footer, reset-credit, font-scale and Codex source layout cases; true AppWidgetHost receives only reveal start/final publications while native capsule transforms support 60/120 Hz timestamps; hardware capsule pixels advance with round caps; native startup begins transparent and eases in gently; six worker-inflated RemoteViews controls reproduce the old empty-bar handoff and verify immediately filled settled pixels in both themes and three layouts; idle/spinning glyph bounds match; hardware spinner changes over elapsed time and stops when hidden; reduced motion and reveal completion/cancellation remain offline; OAuth callback, PKCE, quota/reset parser, encrypted vault and synthetic device-login contracts passed.\n");

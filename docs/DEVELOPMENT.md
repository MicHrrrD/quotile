...
`--check` 只检查已安装的前置环境。普通在线构建可下载并校验固定版本的官方 Gradle；完整缓存后可用 `--offline`。`--java-home`、`--sdk` 和 `--gradle` 支持显式指定路径。输出为 `dist/Quotile-0.3.9.apk`，这是本机构建工具签出的调试 APK。
...
| `docs/images/` | 0.3.8 原生控件实际渲染图，0.3.9 沿用相同外观；均为合成示例数据 |
| `downloads/` | 0.3.9 固定签名安装包与校验信息 |
...
0.3.9 保留既有原生 RemoteViews 布局、文字、颜色、上下留白和横向胶囊条。读取期间使用有时限的原生矢量旋转动画；刷新按钮在设置背景后统一设置 11dp 内边距，使静止和旋转箭头的可见尺寸一致。

成功读取后的约 900ms 展开由桌面宿主中的原生 ViewFlipper 与有限 XML Animation 执行，使用 `cubic-bezier(0.42, 0, 0.58, 1)` 曲线：前 160ms 渐显并缓慢起步，中段平滑展开，最后轻缓收尾，让从无到有的过程更清楚。条身水平缩放，圆形端点以恒定尺寸平移，两端始终保持圆润。双栏使用同一时间进度。应用只发送开始状态，并在约 980ms 后提交最终静态状态；不再逐帧跨进程发送完整 RemoteViews，也不传输包含文字的位图。

0.3.9 修复展开结束时的短暂空条：部分启动器在工作线程创建控件，随后在主线程更新时，底层 ProgressBar 从 0 切到目标进度的 drawable level 更新可能延迟到下一帧，而覆盖层已经隐藏。本版在展开开始时就将底层进度设为真实目标值，并仅将进度填充色设为透明；结束时在同次更新中恢复填充色、隐藏动画覆盖层，避免在交接帧同时改变进度。当前展开采用 900ms 原生动画、约 980ms 收束，并保留开始和结束各一次小组件更新。
...
[0.3.9 的构建与 Android 16 模拟器验证](https://github.com/MicHrrrD/quotile/actions/runs/34010695205)：全部通过。测试源码提交为 `cebf1b9cdb05126c1912caba578771d469a991fc`。本次重点检查真实小组件宿主中动画覆盖层与最终静态进度的交接，验证结束时不会短暂丢失已填充的余量，以及 900ms 缓起缓收曲线的展开行为。

本次 6 组工作线程加载对照均复现旧逻辑的空条，并验证修复后在同一主线程任务内立即绘制完整胶囊。覆盖深浅色、紧凑、每周详情及双栏；延迟收尾也保持填充。160ms 渐显、900ms 缓起步、恒定圆角和 60／120Hz 时间采样全部通过。原有布局、登录、额度及默认手动门禁继续通过，共输出 47 张原生图片，原始结果 `INSTRUMENTATION_CODE: -1`。

本次真实宿主仍只收到开始和结束两次状态发布；约 60Hz 的模拟器记录 61 次 Choreographer 回调，中位间隔 16.666666ms。实际手机帧率仍由系统与桌面决定。
...
0.3.8 检查中的真实 AppWidgetHost 在展开期间只收到开始与最终状态两次更新；宿主中的原生动画持续推进。检查了 60／120Hz 时间采样下的连续变换、恒定圆形端点、最终数值、取消回调及关闭动画时的静态回退。硬件截图确认条形随时间展开，刷新箭头转动且隐藏后停止，静止与旋转箭头的可见边界一致，共输出 44 张原生图片。

0.3.8 检查中的模拟器显示模式约 60Hz，记录到 43 次 Choreographer 回调，中位帧间隔 16.666666ms。这是模拟器的实际调度记录；120Hz 仅验证对应时间点的动画变换，不代表 120Hz 实机帧率测试。运行详情与历史记录见 [VALIDATION.md](../VALIDATION.md)。模拟器通过不能替代所有手机启动器的实机验证；欢迎报告带设备与系统版本的兼容性问题。
